...
class MLP(object):
    ...
    def update_weights(self, iris, hidden_sigmoid, output_sigmoid):
        ...
        for x, value in enumerate(self.hidden_weight):
            for y, _ in enumerate(value):
                sum = 0
                for z in range(3):
                    sum+=(output_sigmoid[z]-iris[4+z])*output_sigmoid[z]*(1-output_sigmoid[z])*old_weights[z][x]
                self.hidden_weight[x][y]-=sum*hidden_sigmoid[x]*(1-hidden_sigmoid[x])*iris[y]*self.learn_rate
    ...
    def run(self):
        ...
        np.savetxt("log08.csv", np.transpose(export), delimiter=",",header="Error, Accuracy")
